-- =============================================================================
-- V_CAP — Cap stratégique par campus (identique à l'Excel bloc ①) — SAP HANA
...
-- =============================================================================
CREATE OR REPLACE VIEW V_CAP AS
SELECT
    n.SCENARIO, n.PERIODE, n.ENTITY,
    n.CAC_MARGINAL, n.CROISS_LEADS, n.INTENSITE_MKT,
    n.INV_CAC      / NULLIF(AVG(n.INV_CAC)      OVER (PARTITION BY n.SCENARIO, n.PERIODE), 0) AS CAP_EFF,
    n.CROISS_LEADS / NULLIF(AVG(n.CROISS_LEADS) OVER (PARTITION BY n.SCENARIO, n.PERIODE), 0) AS CAP_MOM,
    n.INV_INT      / NULLIF(AVG(n.INV_INT)      OVER (PARTITION BY n.SCENARIO, n.PERIODE), 0) AS CAP_POT,
    n.CAP_RETENU,
    n.BUDGET_ACQ_REF,
...
FROM (
    SELECT
        ...
        cp.CAC_MARGINAL,
        COALESCE(sm.LEAD_TOT_26 / NULLIF(sm.LEAD_TOT_24, 0), 0) - 1 AS CROISS_LEADS,
        COALESCE(cp.SPEND_ACQ_REF / NULLIF(sm.CA_26, 0), 0)        AS INTENSITE_MKT,
        1.0            / NULLIF(cp.CAC_MARGINAL, 0)                AS INV_CAC,
        sm.CA_26       / NULLIF(cp.SPEND_ACQ_REF, 0)              AS INV_INT,
        1              AS CAP_RETENU,
