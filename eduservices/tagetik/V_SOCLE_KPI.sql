-- =============================================================================
-- V_SOCLE_KPI (enrichie marketing) — SQL Server
-- =============================================================================
CREATE OR ALTER VIEW V_SOCLE_KPI AS
...
-- IFERROR(x,0) -> COALESCE(1.0 * x / NULLIF(dénominateur,0), 0)
-- =============================================================================
SELECT
    ...
    -- Funnel
    COALESCE(1.0 * t.VOL_EFF / NULLIF(t.VOL_EFF_INF, 0), 0)    AS TX_PASSAGE,
    COALESCE(1.0 * t.VOL_CAND / NULLIF(t.VOL_LEAD,    0), 0)    AS TX_LEAD_CAND,
    COALESCE(1.0 * t.VOL_ADMIS / NULLIF(t.VOL_CAND,    0), 0)    AS TX_CAND_ADMIS,
    COALESCE(1.0 * t.VOL_NEW / NULLIF(t.VOL_ADMIS,   0), 0)    AS TX_YIELD_ADMIS_INSC,

    -- Marketing dérivé
    (t.VOL_LEAD_ORG + t.VOL_LEAD_PAY)                                        AS VOL_LEAD_TOTAL,
    COALESCE(1.0 * t.VOL_LEAD_ORG / NULLIF(t.VOL_LEAD_ORG + t.VOL_LEAD_PAY, 0), 0) AS PART_ORG,
    COALESCE(1.0 * t.DEPENSE_ACQ / NULLIF(t.VOL_LEAD_PAY, 0), 0)                       AS CPL,
